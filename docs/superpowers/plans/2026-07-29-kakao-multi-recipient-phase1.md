...
### 2단계 진행 상황 (2026-07-30)

구현 완료 — `app/api/recipients.py`. `test_recipient_selfregister.py` 39개 항목 통과
(관리자 키 게이트, Redirect URI 미설정 fail-closed, 동의 화면 리다이렉트 파라미터,
논스 1회용성·만료, 위조/누락 state 차단, upsert 신규/갱신, refresh_token 누락·교환 실패·동의
거절 시 행 미생성).

설계 문서 7절과 다르게 구현한 것 하나: `state`를 **서명하지 않는다.** 논스를 서버 dict에
보관하고 콜백에서 소진하는 구조라 값 자체가 서버 발급 난수(`secrets.token_urlsafe(32)`)이며,
저장소에 없는 state는 그대로 거절된다. 서명은 stateless state에 필요한 장치이고 여기서는
검증 경로가 이미 서버 조회이므로 아무것도 더 막아주지 못한다. 수신자 이름도 링크에 싣지 않고
논스 항목에 함께 보관한다.

**남은 것 — 카카오 개발자콘솔 선행작업이 있어야 검증 가능:**

1. 콘솔에 `https://<배포도메인>/api/admin/recipients/callback`을 Redirect URI로 등록
2. `.env`에 같은 값을 `KAKAO_RECIPIENT_REDIRECT_URI=`로 넣기 (미설정 시 `/connect`가 503)
3. 앱이 "개발 중" 상태면 두 번째 수신자를 팀원으로 등록하거나 배포 상태로 전환
4. 그 뒤 실제 초대 링크 왕복 + 두 번째 실제 수신자로 다중 발송 최종 확인
