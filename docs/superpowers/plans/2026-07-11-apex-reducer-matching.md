...
"
```
Expected: `reducer rows: 30`, 14mm 매칭 결과에 `AB060 (1단, 입력홀 ≤14mm...)`, `AB060A (2단...)`, `AB090 (2단...)`, `ABR060 (1단...)` 4건이 나오고, `100mm matches: []`.
...
"
```
Expected: 응답에 `AB060`(1단)/`AB060A`(2단)/`AB090`(2단)/`ABR060`(1단) 매칭 결과와 `🔩 정확한 모터 장착 어댑터(C1~C10)는 APEX 측 확인이 필요합니다.` 문구, 마지막 줄 `PASS`.
...
1~4) HC-KFS053/13/23/43 각각: 응답에 실제 치수 수치가 포함되는지, 치수 면책 문구가
     ...
     전부 실제로 APEX 카탈로그와 매칭되므로(8mm -> AB042/ABR042급, 14mm -> AB060급)
     "감속기 호환 정보 미등록"이 더는 나오면 안 된다.
