...
- HC-KFS053, HC-KFS13 (축경 8mm): 실제 DB 대상 `_match_reducers_by_bore(8.0, ...)` 실행 결과
  최소 등급(표준 ≤11mm급) 6건 — AB042(1단)/AB042(2단)/AB060(2단)/ABR042(1단)/ABR042(2단)/ABR060(2단)
  (AB060/ABR060은 1단은 표준 14mm지만 2단 변형은 표준 11mm로 더 작아짐 — 브레인스토밍
  단계에서 042 계열만 세고 이 두 개를 빠뜨렸던 걸 Task 3 구현 후 실제 DB 조회로 정정)
  → 매칭 결과 + 어댑터 확인 문구 assert.
- HC-KFS23, HC-KFS43 (축경 14mm): 실제 DB 대상 `_match_reducers_by_bore(14.0, ...)` 실행 결과
  최소 등급(입력홀 14mm급) 5건 — AB060(1단)/AB060A(2단)/AB090(2단)/ABR060(1단)/ABR090(2단)
  (ABR090 2단도 표준 14mm — 마찬가지로 초안에서 누락됐다가 정정)
  → 매칭 결과 + 어댑터 확인 문구 assert.
