...
**Low/out-of-stock triggers a Kakao "message to self" admin notification** (`app/services/admin_notify.py`), separate from the older Slack webhook path in `inventory.py`. It's debounced per-model for 1 hour (in-process dict, resets on restart), includes competitor pricing pulled from the Naver Shopping API, and logs to the `StockAlert`/`PriceHistory` tables. The Kakao OAuth token is persisted in the `KakaoToken` DB table (not a file) and refreshed via `refresh_token` on expiry — this only works after a one-time manual OAuth flow (see `kakao_test/kakao_auth_setup.py`).
...
Seven tables under async SQLAlchemy 2.0: `Product` (master, keyed by `model_name`) → `Specification` (1:1, includes a JSON `extra_specs` escape hatch for servo-specific fields like `capacity_w`/`compatible_motors`) and `Inventory` (1:1); `Replacement` (old→new product FK pair with compatibility flags); `StockAlert` and `PriceHistory` (notification/pricing audit trail); `AlarmCode` (manufacturer+series+code lookup, independent of `Product`); `KakaoToken` (singleton row, `id=1`). `app/db/seed.py` auto-populates a small hardcoded sample catalog and the full `IG5A`/`MR-J4`/`FR-E700`/`FR-D700` alarm code tables on startup if the DB is empty — this is why a fresh SQLite file isn't actually empty of useful data after first boot.
