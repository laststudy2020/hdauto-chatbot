...
**Recipients self-register through `app/api/recipients.py`**, a router deliberately kept *outside* `admin.py` because `admin.py`'s router-level `require_admin_key` needs an `X-Admin-Key` header that neither leg of this flow can send — `/api/admin/recipients/connect` is opened by tapping a link in KakaoTalk, and `/api/admin/recipients/callback` is invoked by Kakao's redirect. `connect` gates on a `key` query param instead (compared with `hmac.compare_digest`, fail-closed when `ADMIN_COMMAND_KEY` is unset) and hands out a one-time nonce; `callback` gates on that nonce, which is consumed on entry whether or not registration succeeds, so a leaked invite link can't be replayed. The recipient's name is held server-side in the nonce store rather than round-tripped through the URL. Nonces live in an in-process dict with a 30-minute TTL, so a restart invalidates outstanding invites — acceptable because invites are used immediately. Upsert is keyed on `(name, channel)`, so re-registering under a different name creates a second row. `KAKAO_RECIPIENT_REDIRECT_URI` is a separate setting from the legacy `KAKAO_REDIRECT_URI` (used only by `kakao_test/` scripts) and must match the Kakao developer console's registered redirect URI character-for-character; `connect` returns 503 rather than sending users to a KOE006 error page when it's unset.
