...
from app.services.servo_spec_search import (
    find_servo_by_capacity, find_servo_drive_details, find_drives_compatible_with_motor,
    find_reducer_compat, find_motors_by_reducer,
)
...
async def _route(intent_result, message: str, db: AsyncSession) -> tuple[str, str]:
    ...
    if intent == Intent.REPLACEMENT:
        if model:
            db_reply, matched = await find_replacement(model, db)
            # DB에서 Product/모터 매칭 자체가 실패한 경우(matched=False)만 웹검색으로 폴백.
            ...
            # matched 여부는 함수가 명시적으로 반환하는 값으로 판단하며, CLOVA가 생성한
            # 자유서술 답변에 "찾지 못했습니다" 문구가 우연히 섞여도 오폴백되지 않는다(H6).
            if not matched:
    ...
    if intent == Intent.SPECS:
        if model:
            ...
            if servo_detail is not None:
                reducer_note = await find_reducer_compat(model, db)
                if reducer_note:
                    servo_detail += "\n\n" + reducer_note
            ...
            # 2.5) 감속기 모델명이면 호환되는 서보모터를 역검색
            reducer_reverse = await find_motors_by_reducer(model, db)
            if reducer_reverse is not None:
                return reducer_reverse, "db_reducer_motor_reverse"

            # 3) 그 외 일반 제품 스펙 조회
            db_reply, matched = await lookup_specs(model, db)
            if not matched:
